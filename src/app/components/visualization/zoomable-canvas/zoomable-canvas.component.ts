...
export class ZoomableCanvasComponent implements AfterViewInit {
  ...
  private draw(center: Point, zoomRatio: number): void {
    if (this.context)
    {
  ...
  set zoomLevel(val: number) {
    ...
    const newZoomRatio = Math.pow(2, val);
  ...
  private setCenterX(val: number, redraw: boolean): void {
  ...
  private setCenterY(val: number, redraw: boolean): void {
  ...
  private rescale(previousZoomRatio: number, newZoomRatio: number, center?: Point): void {
  ...
  public mousemove(event: MouseEvent): void {
    if (event.buttons > 0) {
      ...
      const centerXVal = this.centerXValue - dx;
  ...
  public mousewheel(_event: Event|RouterEvent): void {
    ...
    const previousZoomLevel = this.zoomLevelValue;
    const previousZoomRatio = this.zoomRatio;
